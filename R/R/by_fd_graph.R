...
by_fd_graph <- function(obj) {
    ...
    by_graph <- ggplot(data = pe_set, aes(x = .data$x_pos, y = .data$pe, fill = .data$model)) + 
        ...
        panel.grid.minor.y = element_blank(), panel.grid.major.y = element_line(color = "black", size = 0.2)) + geom_hline(yintercept = 0, color = "black", size = 0.2) + labs(fill = "Estimator")
    })
    return(by_graph)
}